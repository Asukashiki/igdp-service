...
public class RowConvertColUtil {
     ...
     **/
    public static synchronized ConvertData doConvertReturnObj(List<?> originalList, String headerName, String[] fixedColumn, String valueFiledName, boolean needHeader, String[] fixedColumnName, String nullValue) throws Exception {
     ...
     * @param nullValue       空值填充
     **/
    public static synchronized List<List<Object>> doConvert(List<?> originalList, String headerName, String[] fixedColumn, String valueFiledName, boolean needHeader, String[] fixedColumnName, String nullValue) throws Exception {
        // 行转列的字段表头
            ...
            for (Object headerItem : headerSet) {
                ...
                for (Object originalObjectItem : originalList) {
                    ...
                    if (headerItem.equals(headerField.get(originalObjectItem))) {
                        ...
                        Field valueField = originalObjectItem.getClass().getDeclaredField(valueFiledName);
                        valueField.setAccessible(true);
                        // 判断当前列是否于固定列的所有值都一样
                        for (int i = 0; i < fixedColumn.length; i++) {
                            fixedColumnField = originalObjectItem.getClass().getDeclaredField(fixedColumn[i]);
                            fixedColumnField.setAccessible(true);
                            if (!fixedColumnItem.toString().split("\\|")[i].equals(fixedColumnField.get(originalObjectItem).toString())) {
                                flg = false;
                            }
                        }
                        if (flg) {
