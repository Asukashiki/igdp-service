...
/**
 * Pre-basic Seed 生产Service实现
 *
 * @author igdp
 */
@Service
public class PrebasicSeedProduceServiceImpl extends ServiceImpl<PrebasicSeedProduceMapper, PrebasicSeedProduce> implements IPrebasicSeedProduceService {
    ...
    @Override
    public List<PrebasicSeedProduceVO> getProduceList(PrebasicSeedProduceQueryDTO queryDTO) {
        return baseMapper.selectProduceList(queryDTO);
    }

    @Override
    public PrebasicSeedProduceVO getProduceById(String produceBatchId) {
        return baseMapper.selectProduceById(produceBatchId);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public PrebasicSeedProduceVO addProduce(PrebasicSeedProduceDTO dto) {
        // 1. 查询来源批次信息（Breeder Seed）
        BreedSeedProduce breederSeed = breedSeedProduceMapper.selectById(dto.getBreederSeedBatchId());
        if (breederSeed == null) {
            throw new ServiceException("Source batch not found");
        }

        // 2. 查询Breeder Seed的生产结果记录并扣减剩余量
        BreedSeedProduceResult produceResult = breedSeedProduceResultMapper.getResultByProduceBatchId(dto.getBreederSeedBatchId());
        if (produceResult == null) {
            throw new ServiceException("Production result not found for batch: " + dto.getBreederSeedBatchId());
        }
        ...
        breedSeedProduceResultMapper.updateById(produceResult);

        // 3. 构建实体对象
        PrebasicSeedProduce produce = new PrebasicSeedProduce();
        
        // 生成批次ID
        String batchId = generateBatchId(breederSeed.getCropType());
        produce.setProduceBatchId(batchId);
        
        // 用户输入字段
        produce.setProduceBatchName(dto.getProduceBatchName());
        produce.setBreederSeedBatchId(dto.getBreederSeedBatchId());
        ...
        // 自动带出字段
        produce.setBreederSeedBatchName(breederSeed.getProduceBatchName());
        produce.setVarietyId(breederSeed.getVarietyId());
        produce.setVarietyName(breederSeed.getVarietyName());
        produce.setCropType(breederSeed.getCropType());
        produce.setBreedBatchId(breederSeed.getBreedBatchId());
        produce.setBreedBatchName(breederSeed.getBreedBatchName());
        produce.setTrialId(breederSeed.getTrialId());
        produce.setTrialName(breederSeed.getTrialName());
        ...
        // 固定值字段
        produce.setFromSeedLevel("Breeder");
        produce.setToSeedLevel("Pre-Basic");
    ...
    public void voidProduce(String produceBatchId) {
        PrebasicSeedProduce produce = baseMapper.selectById(produceBatchId);
        if (produce == null) {
        ...
        // 归还剩余量到Breeder Seed
        BreedSeedProduceResult produceResult = breedSeedProduceResultMapper.getResultByProduceBatchId(produce.getBreederSeedBatchId());
        if (produceResult != null) {
            produceResult.setRemainingQuantity(
