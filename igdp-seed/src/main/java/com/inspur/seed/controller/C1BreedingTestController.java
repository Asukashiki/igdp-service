...
public class C1BreedingTestController {
    ...
    public AjaxResult submit(@PathVariable String id) {
        boolean result = c1BreedingTestService.submit(id);
    ...
    public AjaxResult approve(@RequestBody Map<String, String> params) {
        boolean result = c1BreedingTestService.approve(params.get("id"), params.get("auditComment"));
        return result ? AjaxResult.success("审核通过") : AjaxResult.error("审核失败");
    }

    @PostMapping("/reject")
    public AjaxResult reject(@RequestBody Map<String, String> params) {
        boolean result = c1BreedingTestService.reject(params.get("id"), params.get("auditComment"));
