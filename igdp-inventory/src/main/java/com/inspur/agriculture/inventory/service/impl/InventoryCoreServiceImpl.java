...
public class InventoryCoreServiceImpl implements IInventoryCoreService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void releaseStock(Long productId, Long warehouseId, BigDecimal qty) {
        if (qty == null || qty.compareTo(BigDecimal.ZERO) <= 0) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void reduceStock(Long productId, Long warehouseId, String batchNo, BigDecimal qty) {
        if (qty == null || qty.compareTo(BigDecimal.ZERO) <= 0) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void increaseStock(Long productId, Long warehouseId, String batchNo, BigDecimal qty, Date prodDate, Date expDate) {
        increaseStock(productId, warehouseId, batchNo, qty, prodDate, expDate, null, "AVAILABLE");
    }

    // 重载方法，支持质量等级和状态
    public void increaseStock(Long productId, Long warehouseId, String batchNo, BigDecimal qty, Date prodDate, Date expDate, String qualityGrade, String stockStatus) {
        if (qty == null || qty.compareTo(BigDecimal.ZERO) <= 0) {
    ...
    @Override
    public void reserveStock(Long productId, Long warehouseId, BigDecimal qty) {
        // 暂复用 lockStock
        lockStock(productId, warehouseId, qty);
    }
