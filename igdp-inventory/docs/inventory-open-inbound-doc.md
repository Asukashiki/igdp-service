...
## 业务说明
- 该接口仅创建“待审批”的入库单（状态为 `SUBMITTED`），不会执行实际入库与库存变更。
- 需要审批人员在后台审核通过后，才会执行入库逻辑与库存更新。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| inboundNo | string | 否 | 入库单号（不传可由外部系统生成；系统不自动生成） |
...
{
  "code": 200,
  "msg": "Inbound order created and submitted for approval.",
  "data": 101
}
```
